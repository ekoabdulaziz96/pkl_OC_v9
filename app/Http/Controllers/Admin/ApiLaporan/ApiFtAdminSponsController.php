...
namespace App\Http\Controllers\Admin\ApiLaporan;
...
$users = User::findOrFail($id);
    if ($users->status == 'ft_sponsorship'){
      $laporan = FtSponsorship::where('user_id',$id)->orderBy('created_at','asc')->get();
    }else if ($users->status == 'ft_admin'){
      $laporan = FtAdmin::where('user_id',$id)->orderBy('created_at','asc')->get();
    }
    ...
    ->addColumn('nomor', function(){
        ...
        if ($laporan->status_laporan == 'disetujui'){
          ...
          if($laporan->status_laporan =='baru' || $laporan->status_laporan =='perbaikan'){
           ...
           if (Carbon::now()->lessThan($laporan->expired_at) || $laporan->status_laporan == 'disetujui') {
                 return '<div align="center" >' .
