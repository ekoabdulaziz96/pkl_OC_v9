...
class LaporanAccApiFtAdminSponsController extends Controller
{
  ...
  // $users = User::findOrFail($id);
    if ($status == 'ft_sponsorship'){
      $laporan = FtSponsorship::where('send_ft_kacab',true)->orWhere('send_manajer',true)->orWhere('send_direktur',true)->get();
    }else if ($status == 'ft_admin'){
      $laporan = FtAdmin::where('send_ft_kacab',true)->orWhere('send_manajer',true)->orWhere('send_direktur',true)->get();
    }
  ...
  return Datatables::of($laporan)
    ->addColumn('nomor', function(){
       ...
       $user = User::findOrFail($laporan->user_id);
            return '<span >'.
                    '<a onclick="viewProfil('.$laporan->user_id.')" class="btn btn-default btn-sm" style="background-color:#6C85EF;color:white;width:25%" data-toggle="tooltip" data-placement="top" title="minta perpanjangan deadline laporan"><i class="fa fa-user-o"></i></a> ' .      
                   '</span>&nbsp;&nbsp'.$user->nama;
    ...
    ->rawColumns(['nomor', 'nama','status','cabang','wilayah','created','expired','acc_laporan'])->make(true);
    }
}
