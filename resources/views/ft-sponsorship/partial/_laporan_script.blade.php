...
function addLaporan() {
  ...
  addLaporanSet();
      var user =  $('#id_user').val();
        $.ajax({
            url: "{{ url('ft-sponsorship/laporan-tanggal') }}"+ '/' + user ,
            type: "GET",
            dataType: "JSON",
...
function editLaporan(id) {
  ...
  $('#modal-form form')[0].reset();
  $.ajax({
    url: "{{ url('ft-sponsorship/laporan') }}" + '/' + id + "/edit",
    type: "GET",
    dataType: "JSON",
    success: function(data) {
     editLaporanSet();
...
//change form  
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if(status == 'kacab'){
          ...
          $('#modal-form-acc-title').text('View Acc Laporan oleh Manajer ');
  ...
  function deadlineLaporan(id){
    ...
    swal({
        ...
        $.ajax({
            url: "{{ url('ft-sponsorship/laporan/deadline') }}" + '/' + id,
            type: "GET",
            dataType: "JSON",
            beforeSend: function(){
      ...
      function exportPdfLaporan(id){
         // $.ajax({
         {{-- //    url: "{{ url('ft-sponsorship') }}" + '/'+ {{ $user->id }}+'/laporan/export-pdf/' + id, --}}
         //    type: "GET",
         //  });
         window.open("{{ url('admin') }}" + '/'+ {{ $user->id }}+'/laporan/export-pdf/' + id);
      }      
      ...
      function kirimLaporan(id){
          ...
          swal({
              ...
              $.ajax({
                  url: "{{ url('ft-sponsorship/laporan/kirim') }}" + '/' + id,
                  type: "GET",
                  dataType: "JSON",
                  beforeSend: function(){
                     $("body").css("padding",'0px');
            ...
            $('#modal-form form').validator().on('submit', function (e) {
                ...
                // console.log('cek2');
                    ...
                    // console.log(id+save_method+csrf_token);
                      $.ajax({
                      url : "{{ url('ft-sponsorship/laporan') }}" + '/' + id,
                      type : "POST",
                    ...
                    if (save_method == 'add') url = "{{ url('ft-sponsorship/laporan') }}";
                    else url = "{{ url('ft-sponsorship/laporan') . '/' }}" + id;
