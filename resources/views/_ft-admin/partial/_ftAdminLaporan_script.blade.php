...
function addLaporan() {
  ...
  addLaporanSet();
      var user =  $('#id_user').val();
        $.ajax({
            url: "{{ url('ft-admin/laporan-tanggal') }}"+ '/' + user ,
            type: "GET",
            dataType: "JSON",
...
//change form  
     ...
     function showLaporan(id) {
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            $('#form-show').show();
            $('#form-show-laporan-title').text('Laporan Tanggal '+data.created_at.substring(8,10)+'-'+data.created_at.substring(5,7)+'-'+data.created_at.substring(0,4));
  ...
  function viewAccLaporan(id,status) {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if(status == 'kacab'){
          $('#modal-form-acc-title').text('View Acc Laporan oleh Kepala Cabang ');
          $('#form-acc-show-status').text(data.acc_kacab);
          $('#form-acc-show-komentar').text(data.komentar_kacab);

          $('#modal-header-acc').addClass('modal-header-acc-kacab');
          $('#modal-footer-acc').addClass('modal-footer-acc-kacab'); 
          $('#modal-header-acc').removeClass('modal-header-acc-manager');
          $('#modal-footer-acc').removeClass('modal-footer-acc-manager');
          $('#modal-header-acc').removeClass('modal-header-acc-direktur');
          $('#modal-footer-acc').removeClass('modal-footer-acc-direktur'); 
          
        }else if(status == 'manager'){
          $('#modal-form-acc-title').text('View Acc Laporan oleh Manager ');
          $('#form-acc-show-status').text(data.acc_manager);
          $('#form-acc-show-komentar').text(data.komentar_manager);

          $('#modal-header-acc').removeClass('modal-header-acc-kacab');
          $('#modal-footer-acc').removeClass('modal-footer-acc-kacab'); 
          $('#modal-header-acc').addClass('modal-header-acc-manager');
          $('#modal-footer-acc').addClass('modal-footer-acc-manager');
          $('#modal-header-acc').removeClass('modal-header-acc-direktur');
          $('#modal-footer-acc').removeClass('modal-footer-acc-direktur'); 
        }else if(status == 'direktur'){
          $('#modal-form-acc-title').text('View Acc Laporan oleh Direktur ');
        ...
        $('#form-acc-show-laporan-title').text('Laporan Tanggal '+data.created_at.substring(8,10)+'-'+data.created_at.substring(5,7)+'-'+data.created_at.substring(0,4));
  ...
  function deadlineLaporan(id){
    ...
    swal({
        ...
        $.ajax({
            url: "{{ url('ft-admin/laporan/deadline') }}" + '/' + id,
            type: "GET",
            dataType: "JSON",
            beforeSend: function(){
      ...
      function deleteLaporan(id) {
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            $('#form-input').hide();
            $('#form-delete').show();
            $('#form-delete-text').text('Apakah anda yakin ingin menghapus data pada tanggal '+'"'+data.created_at.substring(8,10)+'-'+data.created_at.substring(5,7)+'-'+data.created_at.substring(0,4)+'"');
            $('#form-show').hide();
      ...
      function exportPdfLaporan(id){
         $.ajax({
            url: "{{ url('ft-admin') }}" + '/'+ {{ $user->id }}+'/laporan/export-pdf/' + id,
            type: "GET",
          });
      }      
      ...
      function kirimLaporan(id){
          ...
          swal({
              ...
              $.ajax({
                  url: "{{ url('ft-admin/laporan/kirim') }}" + '/' + id,
                  type: "GET",
                  dataType: "JSON",
                  beforeSend: function(){
                     $("body").css("padding",'0px');
