  ...
  <div class="box box-success">
    <div class="box-title">
      ...
      <div class="panel-heading">
         <i style="font-size: 20px">Kelola Laporan <u >{{ $user->status}} <b>{{ $status_laporan }}</b></u> </i>
...
@section('script')
 ...
 $("#ftAdminLaporan").attr("class","active");
   if ('{{ $status_laporan }}' == 'baru'){
 $("#ftAdminLaporan_baru").attr("class","active");
  }else    if ('{{ $status_laporan }}' == 'proses'){
 $("#ftAdminLaporan_proses").attr("class","active");
  }else    if ('{{ $status_laporan }}' == 'perbaikan'){
 $("#ftAdminLaporan_perbaikan").attr("class","active");
  }else    if ('{{ $status_laporan }}' == 'disetujui'){
 $("#ftAdminLaporan_disetujui").attr("class","active");
  }
    });
  ...
  @if ($form->tipe == 'file')
    $('#{{$form->slug}}').bind('change', function() {
      if (this.files[0].size >= 5242880){
        alert('ukuran file maks 5 MB, silahkan gunakan file dengan ukuran kurang dr 5 MB');
        $('#{{$form->slug}}').val('');
      }
    });
  @endif
